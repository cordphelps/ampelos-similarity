...
#  R function: `wilcox.test(x, y, paired = TRUE)`.
  ...
  for (i in 1:length(column_names)) {
    ...
    column_repo.tibl <- add_row(column_repo.tibl, filename = parts[[1]], transect = parts[[2]],
                                 week = parts[[3]], row = parts[[4]])
    
  }
  
  
  wilcoxon.SRT.output.tibl <- tibble(
  week = character(),
  am.W = numeric(), 
  am.p = numeric(), 
  pm.W = numeric(), 
  ...
  wilcoxon.1.tibl <- wilcoxon.SRT.tibl %>%
        mutate(transect = "SNH") %>%
        mutate(time = "am") 
  
  wilcoxon.2.tibl <- wilcoxon.SRT.tibl %>%
        mutate(transect = "control") %>%
        mutate(time = "am") 
