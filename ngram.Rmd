...
readr::write_csv(wilcox.test.tibl, filename)
    ...
    setwd("/Users/rcphelps/code/groq/")
    saveGGpng(filename="wilcox.png", subdir="png.output", gg=gg.wilcox)
...
```{r sorensen_correlation, echo=FALSE, include=TRUE, results='asis', message=F, warning=F}
...
si.ngram.url <- c('./metrics/sorensen_combined.csv')

si.ngram.tibl <- dplyr::as_tibble(read.csv(si.ngram.url, header=TRUE, row.names=NULL))

sp <- round(as.numeric(si.ngram.tibl[1, "success_probability", drop = TRUE])/10, digits=3)
nw <- si.ngram.tibl[1, "ngram_width", drop = TRUE]

coeff <- round(cor(si.ngram.tibl$stacked_ngram, si.ngram.tibl$stacked_sim), digits=3)

color_list <- list("royalblue4", "royalblue3", "royalblue2", "darkorange4", "darkorange3", "darkorange2")
    # https://sape.inf.usi.ch/quick-reference/ggplot2/colour

# Keep 5% of the rows (removes 95%)
#si.ngram.tibl <- si.ngram.tibl %>% sample_frac(0.05)
    
    gg.correlation <- ggplot(si.ngram.tibl, aes(x = stacked_ngram, y = stacked_sim)) +
    ...
    geom_smooth(method = "lm", se = TRUE) +
      ...
      geom_boxplot(data=si.ngram.tibl, aes(x = stacked_ngram, y = 0.15), width=0.1) +
      
      geom_boxplot(data=si.ngram.tibl, aes(y = stacked_sim, x = 0.95), width=0.1) +
    
    theme_bw() +
    
    labs(y = "Sørensen Index", 
         x = paste("ngram index",
                   sep=""),
         caption = paste("success probability: ", sp, ", ngram width: ", nw, 
                         "\ncorrelation coefficient: ", coeff,
                         "\nSørensen Index median: ", 
                             round(median(si.ngram.tibl$stacked_sim), digits=3),
                         "\nngram index median: ", 
                             round(median(si.ngram.tibl$stacked_ngram), digits=3),
                         sep="" ))
    
    print(gg.correlation)
    
    setwd("/Users/rcphelps/code/groq/")
    saveGGpng(filename="f.ngram.sorensen.png", subdir="png.output", gg=gg.correlation)
...
# ===============================================================
# data created by spider_lib.py function binomial_credible_interval()
...
for (daytime in list("am", "pm")) {
  ...
  if (daytime == 'pm') {   # inserted to avoid printing 'am' content
    
    library(coda)
    library(bayestestR)
    
    overlap.cl1 <- round(bayestestR::overlap(dist1, dist4), digits=2)
    overlap.cl2 <- round(bayestestR::overlap(dist2, dist5), digits=2)
    overlap.cl3 <- round(bayestestR::overlap(dist3, dist6), digits=2)
    ...
    gg.dist <- ggplot(posterior.tibl) +
      ...
      labs(x = "trap count per observation", 
           y = "trap count relative liklihood",
           caption = paste(
                           "weeks 23-25 (dark), ",
                           "overlap coefficient: ", overlap.cl1,
                           "\nweeks 26-31 (light), ",
                           "overlap coefficient: ", overlap.cl2,
    ...
    setwd("/Users/rcphelps/code/groq/")
    saveGGpng(filename = paste("f.posterior.dist.", daytime, ".png", sep=""), 
            subdir = "png.output", gg = gg.dist)
    ...
    tb <- tibble(
            tr = c(daytime, daytime, daytime),
            week.cl = c('cl1', 'cl2', 'cl3'),
            ci.min.ctl = c(dist4.stats[1], dist5.stats[1], dist6.stats[1]),
            ci.max.ctl = c(dist4.stats[2], dist5.stats[2], dist6.stats[2]),
            ci.min.snh = c(dist1.stats[1], dist2.stats[1], dist3.stats[1]),
            ci.max.snh = c(dist1.stats[2], dist2.stats[2], dist3.stats[2]),
            density.overlap = c(bayestestR::overlap(dist1, dist4),
                                bayestestR::overlap(dist2, dist5),
                                bayestestR::overlap(dist3, dist6) ))
    
    print(tb)
    
    gg.ci <- ggplot() +
    
      geom_pointrange(aes(y = factor('week 23-25'),
      ...
      geom_pointrange(aes(y = factor('week 26-31'),
                            # factor(column_list[9]), 
...
#                      ) +
      ...
      labs( 
            y="SNH / control pair", 
            x="trap count", 
            caption = paste("posterior median and\n 95% credible interval\n", 
                            "daytime: ", daytime, sep="" ) 
            ) 
      ...
      setwd("/Users/rcphelps/code/groq/")
      saveGGpng(filename = paste("f.posterior.credible.interval.", daytime, ".png", sep=""),
...
plotVariance <- function(df1, df2, t) {
  ...
  gg <- ggplot() +
   ...
   #       ) 
      ...
      labs( caption = paste("transect SNH is dark blue, control is light brown", sep="")
          ) 
...
setwd("/Users/rcphelps/code/groq/")
saveGGpng(filename = paste("f.binomial.variance.png", sep=""),
      subdir = "png.output", gg = gg1)
...
source('/Users/rcphelps/code/groq/bug-library.R')
source('/Users/rcphelps/code/groq/overhead.R')
...
gg.no.facets <- densityNoFacets(tibble=input.tibl, periodString="weeks 23-25")
#densityFacets(tibble=input.tibl, periodString="weeks 23-25")
setwd("/Users/rcphelps/code/groq/")
saveGGpng(filename = paste("f.density.facets.23.25.", daytime, ".png", sep=""), 
          subdir = "png.output", gg = gg.no.facets)

input.tibl <- combo.period2.tibl %>% mutate(transect=replace(as.character(transect), transect=='oakMargin', 'SNH')) %>% as_tibble()

gg.no.facets <- densityNoFacets(tibble=input.tibl, periodString="weeks 26-31")
#densityFacets(tibble=input.tibl, periodString="weeks 26-31")
setwd("/Users/rcphelps/code/groq/")
saveGGpng(filename = paste("f.density.facets.26.31.", daytime, ".png", sep=""), 
          subdir = "png.output", gg = gg.no.facets)
...
```{r temporal, echo=FALSE, include=TRUE, results="hide", message=F, warning=T }
...
#
    ...
    week.count.both.tibl <- bugs.tibl %>%
        ...
        dplyr::group_by(transect, week, time) %>%
        dplyr::summarize(crabSpiders=sum(crabSpiders), .groups='drop'  )
    
    week.count.am.tibl <- week.count.both.tibl %>%
      dplyr::filter(time=='am') %>%
      dplyr::filter(transect=='SNH') 
    
    week.count.pm.tibl <- week.count.both.tibl %>%
      dplyr::filter(time=='pm') %>%
      dplyr::filter(transect=='SNH') 

    rawWeekly.SNH <- plotCountsWeeklyV3(day=week.count.am.tibl, night=week.count.pm.tibl, tr="SNH")
    print(rawWeekly.SNH)
    
    setwd("/Users/rcphelps/code/groq/")
    saveGGpng(filename = paste("f.temporal.counts.SNH", ".png", sep=""), 
              subdir = "png.output", gg = rawWeekly.SNH)
    ...
    rawWeekly.control <- plotCountsWeeklyV3(day=week.count.am.tibl, night=week.count.pm.tibl, tr="control")
    print(rawWeekly.control)
    
    setwd("/Users/rcphelps/code/groq/")
    saveGGpng(filename = paste("f.temporal.counts.control", ".png", sep=""), 
              subdir = "png.output", gg = rawWeekly.control)
