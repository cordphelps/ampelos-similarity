...
```{r evaluateNGRAMs, echo=FALSE, include=TRUE, results='asis', message=F, warning=F}
...
source.url <- c("./metrics/NGRAM_row.csv")
...
#0  control    236   pm   34  f f f f f f f f f f   f f f f f f f f T f   f f f f f f f f f T   0.703704  0.703704  0.703704
  ...
  # ------------------------------------------------------------------------
  plotHistBox <- function(tr, time, long.df, short.df) {
    ...
    geom_boxplot(data=short.df, aes(x = value, y = 20), width=1.5) +
    ...
    labs(y = "row pairs assessed (count)", 
         x = "NGRAM computed value\nsame transect, same day row-to-row comparison\n(boxplot excludes NGRAM=1)",
...
for (xsect in list("oakMargin", "control")) {
  ...
  for (daytime in list("am", "pm")) {
    ...
    values.not.equal.one <- length(subset(stacked.tibl, !(value %in% c(1)))$value)
    ...
    stats.df <- plotHistBox(tr = xsect, time = daytime, long.df = stacked.tibl, short.df = not.one.tibl)
...
for (daytime in list("am", "pm")) {
    ...
    gg.dist <- ggplot(posterior.tibl) +
      ...
      labs(x = "animal presence probability for the simulated posterior", 
    ...
    gg.ci <- ggplot() +
      ...
      labs( 
            y="SNH / control pair", 
            x="animal presence probability for the simulated posterior", 
