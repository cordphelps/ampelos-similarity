...
```{r arrangePosteriorAM, echo=FALSE, include=TRUE, results='asis', message=F, warning=F}
...
```{r arrangePosteriorPM, echo=FALSE, include=TRUE, results='asis', message=F, warning=F}
...
plum_gradient <- c("plum3", "plum2", "plum1")
purple_gradient <- c("mediumpurple4", "mediumpurple3", "mediumpurple1")
# ggplot2 Quick Reference: colour (and fill)
#https://sape.inf.usi.ch/quick-reference/ggplot2/colour


color_list <- list("purple3", "purple",  "mediumpurple1", "mediumorchid", "violet", "plum1")
...
plotVariance <- function(df1, df2, t) {
  ...
  new_df <- data.frame(
  oak.variance = oak.tibl$variance,
  control.variance = control.tibl$variance )
  
  new_df$diff <- new_df$oak.variance - new_df$control.variance
