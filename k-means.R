...
kmPlotFromTibble <- function(df, tr, daytime) {
  ...
  gg <- ggplot() +
    ...
    geom_point(data=df, aes(x= as.integer(week), y=position, 
         fill=value), shape = 21, size=5) +
    
    # fill=value), shape = 21, size=5, show.legend = FALSE) +
        ...
        labs(x="week number", 
             y="trap position", 
             caption = paste("transect: ", tr, ", daytime: ", daytime, sep="") ) +
